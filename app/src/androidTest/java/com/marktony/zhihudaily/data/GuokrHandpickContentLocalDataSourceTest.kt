...
@LargeTest
class GuokrHandpickContentLocalDataSourceTest {

    private lateinit var localDataSource: GuokrHandpickContentLocalDataSource
    ...
    fun setup() {
        ...
        GuokrHandpickContentLocalDataSource.clearInstance()
        localDataSource = GuokrHandpickContentLocalDataSource.getInstance(SingleExecutors(), database.guokrHandpickContentDao())
    }
    ...
    fun saveContent_retrievesContent() = runBlockingSilent {
        // Given a piece of new guokr handpick content
        val content = GuokrHandpickContentResult(
                DEFAULT_IMAGE,
        ...
        with(localDataSource) {
            ...
            // Then the guokr handpick content can be retrieved from the persistent repository
            val result = getGuokrHandpickContent(content.id)
            assertThat(result, instanceOf(Result.Success::class.java))
            if (result is Result.Success) {
                assertContent(result.data,
                        DEFAULT_IMAGE,
                        DEFAULT_IS_REPLYABLE,
                        DEFAULT_CHANNELS,
                        DEFAULT_CHANNEL_KEYS,
